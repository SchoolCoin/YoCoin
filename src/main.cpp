...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!IsCoinBase())
        return 0;
    return max(0, (COINBASE_MATURITY+50) - GetDepthInMainChain());
}
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 1000000 blocks
    nSubsidy >>= (nHeight / 1000000); // YoCoin: 1000K 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 24 * 60 * 60; // YoCoin: 4 days
static const int64 nTargetSpacing = 1 * 60; // YoCoin: 1 minute
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "19/Oct/2014 Streptomycin, the first antibiotic remedy for tuberculosis, is isolated by researchers at Rutgers University.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // YoCoin: increase each by adding 2 to bitcoin's value.
